...
export class ProjectService {
  ...
  async initializeProject(projectPath: string, templateId: string): Promise<void> {
    ...
    if (!fs.existsSync(wrapperPath)) {
      ...
      await this.executor.exec('curl', ['-fsSL', '-o', wrapperName, url], projectPath);
      
      if (!isWindows) {
        await this.executor.exec('chmod', ['+x', wrapperName], projectPath);
      }
    }

    // 2. Run init
    const cmd = isWindows ? '.\\amper.bat' : './amper';
    
    // Pass the template ID as an argument
    await this.executor.exec(cmd, ['init', templateId], projectPath);
  }
}
